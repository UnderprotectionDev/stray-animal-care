...
export function EmergencyCTA({ title, description, donateLabel, volunteerLabel }: Props) {
  return (
    <ElectricBorder color="#EF303B" speed={0.8} chaos={0.08} borderRadius={0}>
      <div className="bg-cta text-cta-foreground relative overflow-hidden py-10 px-6 lg:py-14 lg:px-8">
        <DotGridBackground />
        <FloatingPaws paws={CTA_PAWS} color="var(--cta-foreground)" />
        <div className="relative z-10 flex flex-col items-center text-center max-w-2xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-center">
            ...
            <Link
              href="/gonullu-ol"
              className="border-[1.5px] border-cta-foreground text-cta-foreground px-8 py-4 font-bold uppercase text-sm tracking-wider inline-block hover:bg-cta-foreground/10 transition-colors"
            >
              {volunteerLabel}
            </Link>
          </div>
